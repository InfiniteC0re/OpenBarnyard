...
TBOOL SDK_T2GUIFont::Create( SDK_T2GUIFontDef* a_pFontDef )
{
	return CALL_THIS( 0x006c2cf0, SDK_T2GUIFont*, TBOOL, this, SDK_T2GUIFontDef*, a_pFontDef );
}

void SDK_T2GUIFont::DrawTextWrapped( const wchar_t* a_wszText, TFLOAT a_fX, TFLOAT a_fY, TFLOAT a_fWidth, TFLOAT a_fHeight, TUINT32 a_uiColour, TFLOAT a_fScale, TextAlign a_eAlign, void* a_fnCallback /*= TNULL*/ )
{
...
void SDK_T2GUIFont::DrawTextSingleLine( const wchar_t* a_wszText, TINT a_iTextLength, TFLOAT a_fX, TFLOAT a_fY, TUINT32 a_uiColour, TFLOAT a_fScale, void* a_fnCallback /*= TNULL*/ )
{
	...
	TFLOAT         fXToUV        = 0.0f;
	TFLOAT         fYToUV        = 0.0f;
	SDK_T2GUIMaterial* pPrevMaterial = TNULL;
	...
	for ( TINT i = 0; i < a_iTextLength; i++ )
	{
		...
		if ( pFontDef->ui16MinCharacter <= wChar && wChar <= pFontDef->ui16MaxCharacter )
		{
			...
			if ( iCharIndex >= 0 )
			{
				...
				{
					SDK_T2GUIMaterial* pMaterial = ( pFontDef->uiNumMaterials > 1 ) ?
...
TFLOAT SDK_T2GUIFont::GetTextHeightWrapped( const wchar_t* a_wszText, TFLOAT a_fMaxWidth, TFLOAT a_fScale )
{
	auto pFontDef = m_pFontDef;
...
TFLOAT SDK_T2GUIFont::GetTextWidth( const wchar_t* a_wszText, TFLOAT a_fScale )
{
...
TFLOAT SDK_T2GUIFont::GetTextWidth( const wchar_t* a_wszText, TINT a_iTextLength, TFLOAT a_fScale )
{
	TINT iWidth = 0;
