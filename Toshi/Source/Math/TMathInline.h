...
TFORCEINLINE TFLOAT ATan(TFLOAT fVal) { TFLOAT fRes = atanf(fVal); TASSERT(TMath::IsFinite(fRes)); return fRes; }
TFORCEINLINE TFLOAT ATan2(TFLOAT fVal1, TFLOAT fVal2) { TFLOAT fRes = atan2f(fVal1, fVal2); TASSERT(TMath::IsFinite(fRes)); return fRes; }
...
TFORCEINLINE TINT FastMod(TINT a_iNum, TINT a_iModulus) { TASSERT(a_iNum >= 0); TASSERT(a_iModulus > 0); TASSERT(0 == (a_iModulus & (a_iModulus - 1))); return a_iNum & (a_iModulus - 1); }
TFORCEINLINE void SinCos(TFLOAT fVal, TFLOAT& a_rSin, TFLOAT& a_rCos) { a_rSin = sin(fVal); a_rCos = cos(fVal); }
TFORCEINLINE TFLOAT LERP(TFLOAT a, TFLOAT b, TFLOAT t) { return a + t * (b - a); }

template <typename T>
TFORCEINLINE void Clip(T& rVal, const T& Min, const T& Max) { if (Max < rVal) { rVal = Max; } if (rVal < Min) { rVal = Min; } }

template <typename T>
TFORCEINLINE const T& Min(const T& a, const T& b) { if (a < b) { return a; } else { return b; } }

template <typename T>
TFORCEINLINE const T& Max(const T& a, const T& b) { if (a < b) { return b; } else { return a; } }
