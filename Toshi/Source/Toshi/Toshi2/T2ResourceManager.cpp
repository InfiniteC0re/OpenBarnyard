...
void Toshi::T2ResourceManager::IncRefCount(int a_iID)
{
	if (a_iID == T2ResourcePtr::IDINVALID) return;
	m_pData[a_iID].m_iRefCount++;
}

void Toshi::T2ResourceManager::DecRefCount(int a_iID)
{
	if (a_iID == T2ResourcePtr::IDINVALID) return;
	m_pData[a_iID].m_iRefCount--;
...
void Toshi::T2ResourceData::DeInit()
{
	m_iFlags &= ~FLAG_DESTROYED;
	Unload();
...
Toshi::T2ResourceManager::T2ResourceManager(int a_iMaxNumResources)
{
...
Toshi::T2ResourceManager::~T2ResourceManager()
{
	if (this != TNULL)
	{
		delete[] m_pData;
...
void* Toshi::T2ResourceManager::GetData(int a_iID)
{
	TASSERT(a_iID >= 0, "ID must be greater than 0");
	TASSERT(a_iID < m_iMaxNumResources, "");
	TASSERT(TFALSE == m_pData[a_iID].HasFlag(T2ResourceData::FLAG_DESTROYED), "");
...
int Toshi::T2ResourceManager::CreateResource(const char* resourceName, void* unk, t_CreateDestroyCallbk a_fnCreateDestroyCallbk, void* a_pData)
{
	...
	m_pData[iID].m_trb2 = Toshi::TAssetInit::g_pCurrentTRB;

	return iID;
}

void Toshi::T2ResourceManager::DestroyResource(int a_iID)
{
...
void Toshi::T2Resource::CreateResource(const char* resourceName, void* unk, t_CreateDestroyCallbk a_fnCreateDestroyCallbk, void* a_pData)
{
...
void Toshi::T2Resource::DestroyResource()
{
