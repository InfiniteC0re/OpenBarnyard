...
{
	TClass::TClass(
		const char* name, TClassImpl* parent, uint32_t version, size_t size,
	...
	)
	{
		...
		if (m_Parent)
		{
			...
			TClassImpl* tClass = m_Parent->m_LastAttached;
	...
	{
		TClassImpl* tClassProps = m_LastAttached;
	...
	TClass* TClass::FindRecurse(const char* const& name, TClass* parent, bool hasPrevious)
	{
		while (parent)
		{
			TClassImpl* previous = hasPrevious ? parent->m_Previous : nullptr;
