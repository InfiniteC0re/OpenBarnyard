...
Toshi::THashTable::t_ItemHashFunc Toshi::THashTable::DefaultItemHashFunc = [](void* unk, int unk2, int unk3)
{
	...
	while (unk2 != 0)
	{
		...
		{
			hash ^= (hash & 0xF0000000) >> 24 ^ (hash & 0xF0000000);
		}
		buffer++;
		unk2--;
	}
...
void* Toshi::THashTable::Find(void* a_pData)
{
	...
	if (a_pBuckets != TNULL)
	{
		...
		while (a_bucket != -1)
		{
			Item item = m_pSmth[hash];
			bool bRes = m_ItemCompareFunc(a_pData, (void*)(*(int*)m_pItems + m_iItemSize + (int*)item.value), m_iItemSize);
			if (bRes) return &item;
			a_bucket = item.key;
		}
	}
