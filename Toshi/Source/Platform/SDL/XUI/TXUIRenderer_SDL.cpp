...
namespace Toshi {
	...
	void TXUIRenderer::BeginScene()
	{
		...
		auto pRenderContext = TSTATICCAST(TRenderContextSDL*, pRender->GetCurrentRenderContext());
	...
	void TXUIRenderer::SetMaterial(T2GUIMaterial* pMat)
	{
		...
		if (pMat != TNULL)
		{
			...
			//TRenderSDL::Interface()->SetSamplerState(0, pMat->GetSamplerId(), TRUE);
	...
	{
		auto pDisplayParams = TRenderSDL::Interface()->GetCurrentDisplayParams();
