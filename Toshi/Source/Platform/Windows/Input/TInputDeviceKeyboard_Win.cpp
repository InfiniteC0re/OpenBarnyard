...
namespace Toshi {
	...
	TINT TInputDXDeviceKeyboard::ProcessEvents(TEmitter<TInputInterface, TInputInterface::InputEvent>& emitter, float deltaTime)
	{
		if (m_bIsUpdating)
		{
			if (m_bFlag)
...
#pragma warning( default : 4309 )
	...
	TBOOL TInputDXDeviceKeyboard::HandleKeyChange(TEmitter<TInputInterface, TInputInterface::InputEvent>& a_Emitter, TINT a_iKeyIndex, uint8_t a_ui8KeyState)
	{
		...
		else
		{
			...
			TWCHAR* wcsCharacter = TranslateDoodadToCharacter(iDoodad);
	...
	BOOL CALLBACK TInputDXDeviceKeyboard::EnumObjectCallback(LPCDIDEVICEOBJECTINSTANCEA a_poObjectInstance, LPVOID a_pvRef)
	{
		...
		TINT iDoodad = TranslateDXToDoodad(a_poObjectInstance->dwOfs);
		...
		if (iDoodad - 0x20000U < 0x80)
		{
			...
			TWCHAR* wcsKeyName = new TWCHAR[iKeyNameLength + 1];
