...
TBOOL const TInputDXDeviceKeyboard::BindToDIDevice(HWND a_mainWindow, LPCDIDEVICEINSTANCE a_poDeviceInstance, IDirectInputDevice8* a_poDXInputDevice, TBOOL exclusive)
{
    ...
    if (a_mainWindow)
    {
        if (m_poDXInputDevice->SetCooperativeLevel(a_mainWindow, DISCL_FOREGROUND | DISCL_NONEXCLUSIVE) != DI_OK)
        {
            return TFALSE;
        }
    }
